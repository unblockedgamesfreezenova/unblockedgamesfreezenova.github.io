...
function show_leaderboard(data) {
  let html =
    '<table class="table table-striped table-dark"><thead class="thead-dark"><tr><th scope="col">#</th><th scope="col">Username</th><th scope="col">Score</th><th scope="col">Date</th></tr></thead><tbody>';
  let index = 1;
  data.forEach((item) => {
    html +=
      '<tr><th scope="row">' +
...
(function () {
  ...
  function append_content(data) {
    ...
    data.forEach((game) => {
      ...
      if (total_revs > 0) {
        rating =
          Math.round(
            (game["upvote"] / (game["upvote"] + game["downvote"])) * 5
          ) / 10;
      }
      ...
      html +=
        '<div class="list-thumbnail"><img src="' +
        game["thumb_2"] +
        '" class="small-thumb img-rounded" alt="' +
        game["title"] +
        '"></div>';
  ...
  function load_comments(array) {
    ...
    if (!avatar) {
      avatar = "/images/default_profile.webp";
      read_only = true;
    }
    $("#comments").comments({
